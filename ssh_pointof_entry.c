 ...
 *
 * @info: a pointer that holds the parameter and the return info.
 *
 * Return: void
 */
 ...
 *
 * @info: holds the parameter and returns the info.
 *
 * Return: -1 if not exist, 0 if success, 1 if not success, 2 if signals exit()
 */
int hold_builtin(shell_info *info)
{
	int index, builtin_re = -1;
	...
	for (index = 0; builtinstore[index].type; index++)
		...
		{
			info->count_lines++;
			builtin_re = builtinstore[index].func(info);
			break;
		}
	return (builtin_re);
}
...
/**
 * ssh_main - it handles the main shell control.
 *
 * @info: a pointer that holds shell_info struct
 * @av: strings that holds the arguments.
 *
...
/**
 * view_command - get a cmd in PATH or current dir.
 *
 * @info: a pointer that holds parameter and the return info
 *
 * Return: void
 */
void view_command(shell_info *info)
{
	...
	{
		if ((my_cmd(info) || _getenv(info, "PATH=")
			|| info->argv[0][0] == '/') && is_my_exe(info, info->argv[0]))
			fork_process(info);
		else if (*(info->arg) != '\n')
		{
			info->status = 127;
